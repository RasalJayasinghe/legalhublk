...
export function useDocumentSync() {
  ...
  const syncDocuments = useCallback(async (forceRefresh = false) => {
    ...
    try {
      ...
      // Augment with freshest items from per-document directories (for cases when all.json lags)
      try {
        updateProgress("Checking for newer items…", 60);
        ...
        const gazNames: string[] = Array.isArray(listGaz)
          ? listGaz.map((x: any) => x.name as string).filter((n: string) => /^\d{4}-\d{2}-\d{2}-/.test(n))
          : [];
        gazNames.sort().reverse();
        const gazMetaUrls = gazNames.slice(0, 40).map(
          (name) => `https://raw.githubusercontent.com/nuuuwan/lk_legal_docs/main/data/gazettes/${year}/${name}/metadata.json`
        );

        // Extraordinary Gazette directories (e.g., 2448-19); sort numerically descending
        const listExtra: any[] = await fetchJson(`https://api.github.com/repos/nuuuwan/lk_legal_docs/contents/data/extra-gazettes/${year}`);
        const exIds: string[] = Array.isArray(listExtra)
          ? listExtra.map((x: any) => x.name as string).filter((n: string) => /^\d{4}-\d+/.test(n))
          : [];
        exIds.sort((a, b) => {
        ...
        const exMetaUrls = exIds.slice(0, 40).map(
        ...
        const metaResults = await Promise.allSettled(
          metaUrls.map((u) =>
            fetch(u, { cache: "no-store", headers: { Accept: "application/json" } }).then((r) => (r.ok ? r.json() : null))
          )
        );
        ...
        const latestDocs: LegalDocNorm[] = [];
        metaResults.forEach((res: any) => {
          const m = res?.value;
          if (!m || !m.id || !m.date) return;
          const title = m.description || "Untitled";
          const pdfUrl = m.lang_to_source_url?.en || m.lang_to_source_url?.si || m.lang_to_source_url?.ta || "";
          latestDocs.push({
            id: m.doc_num || m.id,
            title,
            date: m.date,
            type: typeMap[m.doc_type_name] || m.doc_type_name,
            summary: title,
            pdfUrl,
            rawTypeName: m.doc_type_name,
          });
        });

        if (latestDocs.length) {
          const byId = new Map<string, LegalDocNorm>();
          docs.forEach((d) => byId.set(d.id, d));
          latestDocs.forEach((d) => byId.set(d.id, d));
        ...
        console.warn("Augmenting with latest directories failed", e);
      }
